export const IS_CREATING_EXPENSE = 'IS_CREATING_EXPENSE';
export const IS_AUTHENTICATING = 'IS_AUTHENTICATING';
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS';
export const REGISTER_FAILED = 'REGISTER_FAILED';
export const GET_ALL_ERRORS = 'GET_ALL_ERRORS';
export const CLEAR_ERRORS = 'CLEAR_ERRORS';
export const LOGIN_FAILED = 'LOGIN_FAILED';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGOUT_SUCCESSFULL = 'LOGOUT_SUCCESSFULL';
export const IS_FETCHING_EXPENSES = 'IS_FETCHING_EXPENSES';
export const FETCH_EXPENSES_SUCCESS = 'FETCH_EXPENSES_SUCCESS';
export const FETCH_EXPENSES_FAILED = 'FETCH_EXPENSES_FAILED';
export const CREATE_EXPENSE_FAILED = 'CREATE_EXPENSE_FAILED';
export const CREATE_EXPENSE_SUCCESS = 'CREATE_EXPENSE_SUCCESS';
export const CLEAR_EXPENSE_ADDED = 'CLEAR_EXPENSE_ADDED';
export const DELETE_EXPENSE_SUCCESS = 'DELETE_EXPENSE_SUCCESS';
export const DELETE_EXPENSE_FAILED = 'DELETE_EXPENSE_FAILED';
export const IS_DELETING_EXPENSE = 'IS_DELETING_EXPENSE';
export const CLEAR_EXPENSE_DELETED = 'CLEAR_EXPENSE_DELETED';
export const EDIT_EXPENSE_FAILED = 'EDIT_EXPENSE_FAILED';
export const EDIT_EXPENSE_SUCCESS = 'EDIT_EXPENSE_SUCCESS';
export const IS_EDITING_AN_EXPENSE = 'IS_EDITING_AN_EXPENSE';
export const RESET_PASSWORD_EMAIL_SEND_FAILED = 'RESET_PASSWORD_SENT_FAILED';
export const RESET_PASSWORD_EMAIL_SEND_SUCCESS =
  'RESET_PASSWORD_EMAIL_SEND_SUCCESS';
export const IS_SENDING_RESET_EMAIL = 'IS_SENDING_RESET_EMAIL';
